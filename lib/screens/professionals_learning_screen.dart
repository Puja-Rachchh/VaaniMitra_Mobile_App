...
class _ProfessionalsLearningScreenState extends State<ProfessionalsLearningScreen> {
  ...
  Future<void> _loadProfessionalDescription() async {
    if (professionals.isNotEmpty && currentProfessionalIndex < professionals.length) {
      ...
      final description = 'This is a ${currentProfessional['englishName']}. They work in different fields to serve the community.';
      if (mounted) {
        setState(() {
  ...
  Future<void> _initializeSpeechRecognition() async {
    debugPrint('ProfessionalsLearningScreen: Initializing speech recognition...');
    try {
      final initialized = await SpeechRecognitionService.initialize();
      debugPrint('ProfessionalsLearningScreen: Speech recognition initialization result: $initialized');
      if (mounted) {
        setState(() {
      ...
      if (initialized) {
        debugPrint('ProfessionalsLearningScreen: Speech recognition initialized successfully');
      } else {
        debugPrint('ProfessionalsLearningScreen: Speech recognition initialization failed');
      }
    } catch (e) {
      debugPrint('ProfessionalsLearningScreen: Error initializing speech recognition: $e');
      if (mounted) {
        setState(() {
  ...
  Future<void> _startListening() async {
    debugPrint('ProfessionalsLearningScreen: Attempting to start listening...');
    
    if (!_speechInitialized) {
      debugPrint('ProfessionalsLearningScreen: Speech not initialized, attempting reinitialization...');
    ...
    if (targetLanguage == null) {
      debugPrint('ProfessionalsLearningScreen: Target language is null');
    ...
    try {
      debugPrint('ProfessionalsLearningScreen: Starting speech recognition for language: $targetLanguage');
      final success = await SpeechRecognitionService.startListening(
        languageCode: targetLanguage!,
        onResult: (text) {
          debugPrint('ProfessionalsLearningScreen: Recognized text: $text');
          if (mounted) {
            setState(() {
      ...
      if (!success) {
        debugPrint('ProfessionalsLearningScreen: Failed to start speech recognition');
        if (mounted) {
          setState(() {
            _isListening = false;
          });
      ...
      debugPrint('ProfessionalsLearningScreen: Error starting speech recognition: $e');
      if (mounted) {
        setState(() {
          _isListening = false;
        });
  ...
  void _checkPronunciation() {
    if (_recognizedText.isEmpty || currentProfessionalIndex >= professionals.length) return;

    final expectedWord = professionals[currentProfessionalIndex]['targetName']!;
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => Scaffold(
          appBar: AppBar(
            title: const Text('Professionals Quiz'),
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      Text(
                        'Tap the button and say: ${professionals[currentProfessionalIndex]['targetName']}',
                        style: const TextStyle(fontSize: 14),
                        textAlign: TextAlign.center,
                      ),
                      const SizedBox(height: 12),
